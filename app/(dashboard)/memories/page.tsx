...
import { ArrowLeftIcon, PlusIcon, TrashIcon, EditIcon, SearchIcon, BrainIcon, XIcon } from "lucide-react";
...
export default function MemoriesPage() {
  ...
  useEffect(() => { setLoading(true); fetchMemories(); }, [fetchMemories]);

  const handleAdd = useCallback(async () => {
    if (!newContent.trim()) { toast.error("请输入记忆内容"); return; }
    try {
      const res = await fetch("/api/memories", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ content: newContent.trim(), category: newCategory, importance: newImportance }) });
      if (res.ok) { toast.success("记忆已添加"); setAddDialogOpen(false); setNewContent(""); setNewCategory("fact"); setNewImportance(5); fetchMemories(); } else { const err = await res.json().catch(() => ({})); toast.error(err.error || "添加失败"); }
  ...
  const handleEdit = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/memories", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: editingMemory.id, content: editContent.trim(), category: editCategory, importance: editImportance }) });
      if (res.ok) { toast.success("记忆已更新"); setEditDialogOpen(false); setEditingMemory(null); fetchMemories(); } else { toast.error("更新失败"); }
    } catch { toast.error("网络错误"); }
  }, [editingMemory, editContent, editCategory, editImportance, fetchMemories]);

  const handleDelete = useCallback(async (id: string) => {
    try { const res = await fetch(`/api/memories?id=${id}`, { method: "DELETE" }); if (res.ok) { toast.success("记忆已删除"); fetchMemories(); } else { toast.error("删除失败"); } } catch { toast.error("网络错误"); }
  }, [fetchMemories]);
      ...
      <div className="mx-auto max-w-2xl px-6 py-8">
        <div className="mb-8 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-foreground">记忆管理</h1>
              <p className="text-xs text-muted-foreground/80">AI 从你的对话中提取的关键信息</p>
            </div>
          </div>
          <Button size="sm" onClick={() => setAddDialogOpen(true)} className="bg-primary text-primary-foreground hover:bg-primary/90 rounded-xl">
        ...
        <div className="mb-5">
          <div className="relative">
            ...
            <Input className="pl-8 bg-white/[0.04] border-white/[0.06] text-foreground placeholder:text-muted-foreground/60 rounded-xl text-xs" placeholder="搜索记忆..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
          </div>
        </div>

        <div className="mb-5 flex flex-wrap gap-2">
          {CATEGORIES.map((cat) => (
            <Badge key={cat.value} variant={filterCategory === cat.value ? "default" : "outline"} className={`cursor-pointer text-xs rounded-xl border-white/[0.06] data-[state=active]:bg-white/[0.08] data-[state=active]:text-foreground data-[state=active]:border-white/[0.15] text-muted-foreground/80 ${filterCategory !== cat.value ? "" : ""}`} onClick={() => setFilterCategory(cat.value)}>
              {cat.label}
            </Badge>
          ))}
        </div>
        ...
        {loading ? (
          <div className="flex justify-center py-16"><div className="size-6 animate-spin rounded-full border-2 border-white/10 border-t-white/50" /></div>
        ) : memories.length === 0 ? (
          ...
          <div className="space-y-2">
            {memories.map((mem) => {
              ...
              return (
                <div key={mem.id} className="glass-card p-4">
        ...
        <Dialog open={addDialogOpen} onOpenChange={setAddDialogOpen}>
          <DialogContent className="glass-dialog">
            <DialogHeader><DialogTitle className="text-foreground">添加记忆</DialogTitle></DialogHeader>
        ...
        <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
          <DialogContent className="glass-dialog">
            <DialogHeader><DialogTitle className="text-foreground">编辑记忆</DialogTitle></DialogHeader>
            <div className="space-y-3">
