...
Compass 支持注册自定义 AI 工具（Skill），扩展 AI 的能力。以下为内置 Skill：

### 旅游分析 — 全球旅游目的地分析助手

针对用户输入的旅游目的地，全面分析：预算费用估算、必游人文景观、实用避雷攻略，输出精美 HTML 报告。

**输入参数：**

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `destination` | string | 是 | 旅游目的地，如"日本东京"、"法国巴黎" |
| `duration` | integer | 是 | 旅游天数 |
| `travel_style` | select | 是 | 旅游风格：budget（穷游）/ mid_range（中档）/ luxury（豪华） |
| `travelers` | integer | 否 | 出行人数，默认 1 |
| `interests` | multiselect | 否 | 兴趣偏好列表 |
| `depart_city` | string | 否 | 出发城市，用于机票估算 |

**执行流程：**

1. **目的地信息收集** — 搜索目的地的地理、气候、语言、货币、最佳旅游季节、当前热度与口碑
2. **费用预算分析** — 估算机票、住宿、餐饮、交通、门票、购物等费用，汇总为人民币总预算
3. **人文景观分析** — 整理 TOP 5~8 必游景点、特色美食、文化习俗、最佳线路
4. **避雷攻略** — 搜索常见宰客套路、踩坑景点、交通/餐饮/购物注意事项、安全提示
5. **生成 HTML 报告** — 输出精美中文报告，含费用卡片、景点推荐、行程安排、避雷警告
6. **记录 Session** — 调用 `record_session` 记录本次分析数据

**报告内容包含：**
- 顶部 Banner：目的地名称、旅行概览标签
- 费用预算卡片：分项费用表格 + 总预算高亮
- 景点推荐卡片：配图标、评分、简介
- 推荐行程：按天列出行程安排
- 避雷红色警告区：醒目展示各类坑点
- 实用信息：货币 / 签证 / 紧急电话 / 最佳季节

**输出：** 以 HTML artifact 输出，路径 `files/travel_report.html`
